...
var skillsArray = [];
var skillsArrayReady = [];
var namesArray = [];
var namesArrayReady = [];
var friendsArray = [];
var friendsArrayReady = [];

// реализация через цикл

// for (var i=0; i<dataGot.length; i++) {
  // for (var j=0; j<dataGot[i].skills.length; j++) {
    // skillsArray.push(dataGot[i].skills[j]);
  // };
  // namesArray.push({name: dataGot[i].name, friendsNumb: dataGot[i].friends.length});
  // for (var k=0; k<dataGot[i].friends.length; k++) {
    // friendsArray.push(dataGot[i].friends[k].name);
  // }
// }

// реализация через _.forEach

_.forEach(dataGot, function(value) {
  namesArray.push({'name': value.name, 'friendsNumb': value.friends.length});
  _.forEach(value.skills, function(skillsValue) {
    skillsArray.push(skillsValue);
  });
  _.forEach(value.friends, function(friendsValue) {
    friendsArray.push(friendsValue.name);
  });
});

skillsArrayReady = _.sortBy(_.uniq(skillsArray));
console.log(skillsArrayReady);
namesArrayReady = _.sortBy(namesArray, 'friendsNumb');
console.log(namesArrayReady);
friendsArrayReady = _.uniq(friendsArray);
console.log(friendsArrayReady);
